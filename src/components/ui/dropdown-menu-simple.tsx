...
export const DropdownMenu = component$<DropdownMenuProps>(({ class: className }) => {
  ...
  return (
    <div class={cn('relative inline-block text-left', className)} data-dropdown>
      <Slot toggle={toggle} close={close} isOpen={isOpen.value} />
    </div>
  )
})
...
export const DropdownMenuTrigger = component$<DropdownMenuTriggerProps>(({ class: className, toggle }) => {
  return (
    <button
      type="button"
      class={cn(
...
export const DropdownMenuContent = component$<DropdownMenuContentProps>(({ class: className, align = 'end', isOpen = false }) => {
